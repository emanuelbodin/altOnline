...
def tablesInit():
    ...
    mycursor.execute("CREATE TABLE IF NOT EXISTS order_items (product_title varchar(255), orderID int(11), quantity int(11), current_prodyct_price int(11), current_product_discount int(11), current_product_tax int(11), CONSTRAINT PK_ORDERITEMS PRIMARY KEY (product_title, orderID))")
    mycursor.execute("CREATE TABLE IF NOT EXISTS reviews (review_product varchar(255), email varchar(255), score int(11), review_text text(100), CONSTRAINT PK_REVIEWS PRIMARY KEY (review_product, email), FOREIGN KEY (review_product) REFERENCES products(title))")
